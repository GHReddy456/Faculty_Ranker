import json, re
import difflib

def normalize(name):
    n = name.lower()
    n = re.sub(r'dr\.?\s*|ms\.?\s*|mr\.?\s*|prof\.?\s*', '', n)
    n = re.sub(r'[^a-z\s]', ' ', n)
    n = re.sub(r'\s+', ' ', n).strip()
    return n

print("Loading api_data.json...")
with open('api_data.json', 'r', encoding='utf-8') as f:
    data = json.load(f)

api_faculty = []
api_by_norm = {}

for call in data:
    if 'data' in call and 'data' in call['data']:
        items = call['data']['data']
        if isinstance(items, list):
            for item in items:
                attrs = item.get('attributes', item)
                name = attrs.get('Name', '').strip()
                if name:
                    api_faculty.append(name)
                    api_by_norm[normalize(name)] = name

with open('../src/app/showFaculty/query_faculty.ts', 'r', encoding='utf-8') as f:
    ts_content = f.read()

blocks = re.findall(r'\{[^{}]+\}', ts_content, re.DOTALL)
ts_faculty = []
for b in blocks:
    name_m = re.search(r'"name":\s*"([^"]+)"', b)
    if name_m:
        ts_faculty.append(name_m.group(1))

# Re-run existing matching to isolate the remaining unmatched ones
matched = set()
for ts_name in ts_faculty:
    ts_norm = normalize(ts_name)
    if ts_norm in api_by_norm:
        matched.add(ts_name)
        continue
    
    # Word match
    ts_words = set(w for w in ts_norm.split() if len(w) > 2)
    best_score = 0
    best_match = None
    for api_name in api_faculty:
        api_norm = normalize(api_name)
        api_words = set(w for w in api_norm.split() if len(w) > 2)
        common = ts_words & api_words
        if len(common) >= 2:
            score = len(common) / max(len(ts_words), len(api_words), 1)
            if score > best_score:
                best_score = score
                best_match = api_name
    if best_match and best_score >= 0.55:
        matched.add(ts_name)
        continue
        
    # Subset match
    subset_matches = []
    for api_name in api_faculty:
        api_norm = normalize(api_name)
        api_w = set(w for w in api_norm.split() if len(w) > 2)
        if len(api_w) == 0 or len(ts_words) == 0:
            continue
        if api_w.issubset(ts_words) or ts_words.issubset(api_w):
            subset_matches.append(api_name)
    if len(subset_matches) == 1:
        matched.add(ts_name)

unmatched = [n for n in ts_faculty if n not in matched]
print(f"Unmatched count: {len(unmatched)}")

# Now for the remaining, let's do a strict difflib character matching
print("\n--- Potential Misspellings ---")
for ts_name in unmatched:
    ts_norm = normalize(ts_name)
    if len(ts_norm) < 3: continue
    
    best_ratio = 0
    best_match = None
    for api_name in api_faculty:
        api_norm = normalize(api_name)
        ratio = difflib.SequenceMatcher(None, ts_norm, api_norm).ratio()
        if ratio > best_ratio:
            best_ratio = ratio
            best_match = api_name
            
    if best_ratio > 0.70:
        print(f"Match: '{ts_name}'  <--->  '{best_match}' (Score: {best_ratio:.2f})")
